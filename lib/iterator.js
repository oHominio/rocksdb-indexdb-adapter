...
export class Iterator extends Readable {
  ...
  async _loadEntriesAhead(db, storeName, keyRange) {
    ...
    try {
      ...
      const targetStore = useSnapshotStore ? this._options.snapshotStoreName : storeName;
      ...
      for (let i = 0; i < this._allEntries.length; i++) {
        ...
        if (this._keyEncoding || this._valueEncoding) {
          ...
          this._allEntries[i] = {
            key: Buffer.isBuffer(entry.key) ? entry.key : bufferFrom(String(entry.key)),
            value: Buffer.isBuffer(entry.value) ? entry.value : bufferFrom(String(entry.value))
          };
        }
      }
    } catch (err) {
