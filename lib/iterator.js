...
// Add helper function for more reliable entry sorting
function compareEntries(a, b, isReverse = false) {
...
export class Iterator extends Readable {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        try {
          ...
          // First try to get all entries from the store to see what's there
          store.getAll().onsuccess = (event) => {
            // Now open the cursor with our range
          ...
          console.error('ERROR in _loadEntriesAhead inner try block:', innerErr);
          reject(innerErr);
        }
      });
      
      // Sort entries according to options if needed
      this._sortEntries();
      
      // Limit entries if needed
      if (this._limit > 0 && this._allEntries.length > this._limit) {
        this._allEntries = this._allEntries.slice(0, this._limit);
      }
    } catch (err) {
      console.error('ERROR in _loadEntriesAhead:', err);
      this._error = err;
    }
  }
    ...
    return {
      async next() {
        ...
        try {
          ...
          if (!self._allEntries || self._entryIndex >= self._allEntries.length) {
            return { done: true };
          }
  ...
  _decodeKey(key) {
    ...
    if (this._keyEncoding === 'utf8' || this._keyEncoding === 'ascii') {
      ...
      return typeof key === 'string' ? key : Buffer.from(key).toString(this._keyEncoding);
    } 
    ...
    else if (this._keyEncoding === 'binary' || this._keyEncoding === 'buffer') {
    ...
    if (typeof this._keyEncoding === 'object' && this._keyEncoding.decode) {
      try {
        return this._keyEncoding.decode(Buffer.isBuffer(key) ? key : Buffer.from(String(key)));
      } catch (err) {
  ...
  _decodeValue(value) {
    ...
    if (this._valueEncoding === 'utf8' || this._valueEncoding === 'ascii') {
      // Return string for string encodings
      return typeof value === 'string' ? value : Buffer.from(value).toString(this._valueEncoding);
    } 
    ...
    else if (this._valueEncoding === 'binary' || this._valueEncoding === 'buffer') {
      // Convert to Buffer for binary encodings
      return Buffer.isBuffer(value) ? value : Buffer.from(String(value));
    }
    ...
    if (typeof this._valueEncoding === 'object' && this._valueEncoding.decode) {
      try {
        return this._valueEncoding.decode(Buffer.isBuffer(value) ? value : Buffer.from(String(value)));
      } catch (err) {
  ...
  _getKeyRange() {
    ...
    try {
      if (lowerBound !== undefined && upperBound !== undefined) {
        return IDBKeyRange.bound(lowerBound, upperBound, lowerExclusive, upperExclusive);
  ...
  _encodeKey(key) {
    ...
    try {
      ...
      if (typeof this._keyEncoding === 'object' && this._keyEncoding.encode) {
        ...
        const buf = Buffer.alloc(this._keyEncoding.encodingLength ? 
                               this._keyEncoding.encodingLength(key) : 
                               8192); // pre-allocate reasonable buffer
        const state = { start: 0, end: buf.length, buffer: buf };
      ...
      if (this._keyEncoding === 'utf8' || this._keyEncoding === 'json' || this._keyEncoding === 'ascii') {
