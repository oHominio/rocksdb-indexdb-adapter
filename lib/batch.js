...
class RocksDBReadBatch extends RocksDBBatch {
  _init() {
  ...
  async _flush() {
    ...
    try {
      const snapshotHandle = this._db._snapshot
        ? this._db._snapshot._handle
        : null;
  ...
  _onread(errs, values) {
    ...
    for (let i = 0, n = this._promises.length; i < n; i++) {
      ...
      if (err) {
        ...
        if (values[i]) {
          const valueBuffer = Buffer.isBuffer(values[i])
            ? values[i]
            : Buffer.from(values[i]);
          decodedValue = this._decodeValue(valueBuffer);
        }
        promise.resolve(decodedValue);
      }
    }
...
class RocksDBWriteBatch extends RocksDBBatch {
  _init() {
  ...
  tryDelete(key) {
    ...
    this._operations.push(
      new RocksDBDel(this._encodeKey(key), this._db._columnFamily)
    );
  ...
  put(key, value) {
    return this.tryPut(key, value);
  }

  delete(key) {
    return this.tryDelete(key);
  }

  deleteRange(start, end) {
    return this.tryDeleteRange(start, end);
  }
}

export {
  RocksDBGet,
  RocksDBReadBatch as ReadBatch,
  RocksDBWriteBatch as WriteBatch,
};
