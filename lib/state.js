...
export class State {
    ...
    if (columnFamilies && Array.isArray(columnFamilies)) {
      for (const cf of columnFamilies) {
        ...
        if (!this._columnFamilies.has(cfName)) {
          const columnFamily =
            typeof cf === "string" ? { name: cf, _handle: { name: cf } } : cf;
  ...
  async createReadBatch(db, options) {
    console.log("Creating read batch with options:", options);
    ...
    if (this._readBatches.length > 0) {
      ...
      // Ensure batch has all necessary methods before returning
      this._ensureBatchMethods(batch, false);
      console.log("Reusing existing read batch");
      return batch;
    }

    // Import dynamically to avoid circular dependencies
    const { Batch } = await import("./batch.js");
    const batch = new Batch(db, { ...options, write: false });

    // Ensure batch has all necessary methods
    this._ensureBatchMethods(batch, false);
    console.log("Created new read batch");
    return batch;
  }
  ...
  async createWriteBatch(db, options = {}) {
    console.log("Creating write batch with options:", options);
    ...
    if (this._writeBatches.length > 0) {
      ...
      // Ensure batch has all necessary methods before returning
      this._ensureBatchMethods(batch, true);
      console.log("Reusing existing write batch");
      return batch;
    }

    // Import dynamically to avoid circular dependencies
    const { Batch } = await import("./batch.js");
    const batch = new Batch(db, { ...options, write: true });

    // Ensure batch has all necessary methods
    this._ensureBatchMethods(batch, true);
    console.log("Created new write batch");
    return batch;
  }

  /**
   * Ensure a batch has all required methods
   * @private
   * @param {Batch} batch - The batch to ensure methods on
   * @param {boolean} isWrite - Whether this is a write batch
   */
  _ensureBatchMethods(batch, isWrite) {
    if (!batch) return;

    // Define safe fallback methods if they're missing
    const noop = () => batch;
    const asyncNoop = () => Promise.resolve();

    // Ensure Read methods
    if (typeof batch.get !== "function") {
      console.log("Adding missing get method to batch");
      batch.get = async (key) => {
        try {
          console.log("Using patched get method on batch");
          if (!batch.db || !batch.db._state || !batch.db._state._db)
            return null;

          const storeName = batch._getCfName
            ? batch._getCfName()
            : batch.db._columnFamily || "default";

          const db = batch.db._state._db;
          ...
          return new Promise((resolve) => {
            ...
            req.onsuccess = () =>
              resolve(req.result ? Buffer.from(String(req.result)) : null);
            req.onerror = () => resolve(null);
          });
        } catch (err) {
          console.error("Error in patched get method:", err);
          return null;
        }
      };
    }
    ...
    if (typeof batch.tryFlush !== "function") {
      console.log("Adding missing tryFlush method to batch");
      batch.tryFlush = async () => {
        try {
          console.log("Using patched tryFlush method");

          // For read batches, just do cleanup
          if (!isWrite && typeof batch.destroy === "function") {
            batch.destroy();
          }

          // For write batches, flush pending operations
          if (isWrite && typeof batch.flush === "function") {
            try {
              batch.flush().catch((err) => {
                console.error("Error in patched tryFlush:", err);
              });
            } catch (err) {
              console.error("Error calling flush in patched tryFlush:", err);
            }
          }
        } catch (err) {
          console.error("Error in patched tryFlush method:", err);
        }
    ...
    if (isWrite) {
      if (typeof batch.put !== "function") {
        console.log("Adding missing put method to write batch");
        batch.put = (key, value) => {
          try {
            console.log("Using patched put method");
            if (!batch._pendingOps) batch._pendingOps = new Map();

            const keyStr = typeof key === "string" ? key : key.toString();
            batch._pendingOps.set(keyStr, { type: "put", key: keyStr, value });
          } catch (err) {
            console.error("Error in patched put method:", err);
          }
          return batch;
        };
      }
      ...
      if (typeof batch.delete !== "function") {
        console.log("Adding missing delete method to write batch");
        batch.delete = (key) => {
          try {
            console.log("Using patched delete method");
            if (!batch._pendingOps) batch._pendingOps = new Map();

            const keyStr = typeof key === "string" ? key : key.toString();
            batch._pendingOps.set(keyStr, { type: "delete", key: keyStr });
          } catch (err) {
            console.error("Error in patched delete method:", err);
          }
          return batch;
        };
      }
      ...
      if (typeof batch.flush !== "function") {
        console.log("Adding missing flush method to write batch");
        batch.flush = async () => {
          try {
            console.log("Using patched flush method");
            if (
              batch._processPendingOps &&
              typeof batch._processPendingOps === "function"
            ) {
              await batch._processPendingOps();
            } else {
              // Direct implementation in case _processPendingOps is missing
              if (
                batch._pendingOps &&
                batch._pendingOps.size > 0 &&
                batch.db &&
                batch.db._state &&
                batch.db._state._db
              ) {
                ...
                for (const op of batch._pendingOps.values()) {
                  if (op.type === "put") {
                ...
                batch._pendingOps.clear();
              }
            }
          } catch (err) {
            console.error("Error in patched flush method:", err);
          }
        };
      }
    }
      ...
      const db = await new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
          ...
          if (!db.objectStoreNames.contains("default")) {
            console.log(
              `Creating object store 'default' in database '${dbName}'`
            );
          ...
          if (this._options && this._options.columnFamilies) {
            this._options.columnFamilies.forEach((cfName) => {
              if (!db.objectStoreNames.contains(cfName)) {
                console.log(
                  `Creating object store '${cfName}' in database '${dbName}'`
                );
    ...
    try {
      ...
      if (this._db) {
        ...
        missingColumnFamilies.push(
          ...columnFamilies.filter(
            (name) => !this._db.objectStoreNames.contains(name)
          )
        );
      ...
      if (err.name === "VersionError") {
        console.warn(
          "Database already open in another connection - cannot upgrade"
        );
