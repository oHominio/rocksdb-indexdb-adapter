...
export class State {
   ...
   */
  close() {
    // IDB databases are automatically closed when all connections are closed
    // Just mark it as closed for our internal state
    this.db = null;
    this.opened = false;
    this.closed = true;
  }
   ...
   */
  async suspend() {
    if (this._suspended) return this._suspended;
    if (this._suspending) return this._suspending;
    
    // Create a new deferred promise for suspension
    const deferred = defer();
    this._suspended = deferred.promise;
    this._suspendCallback = deferred.resolve;
    
    // Emit suspend event
    this.emit('suspend');
    
    // Don't wait for anything, just return immediately
    // This matches RocksDB behavior which doesn't actually wait
    return Promise.resolve();
  }
   ...
   */
  async _suspend() {
    // For compatibility, but we don't use this anymore
    return Promise.resolve();
  }
  ...
  resume() {
    ...
    const suspended = this._suspended;
    this._suspended = null;
    
    // Emit resume event
    this.emit('resume');
    
    // Resolve suspended operations
    if (this._suspendCallback) {
      this._suspendCallback();
      this._suspendCallback = null;
    }
    
    // Store the resumed promise for tests
    this.resumed = suspended;
